...
// PATCH: Update status of a submitted contact request by ID
router.patch('/:id', async (req, res) => {
    try {
        ...
        const result = await ContactUsSubmittedCollection.updateOne(
            { _id: new ObjectId(id) },
        ...
        console.error('Error updating submitted contact request status:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error',
            error: error.message
